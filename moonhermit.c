...
#define errl(code, msg) \
	lua_pushstring(L, msg); \
	lua_error(L); \
	return 0;

void errlx(lua_State *L, char *msg, const char *var) {
	lua_pushstring(L, msg); 
	lua_error(L);
}
...
static int muxrpc_read_async(lua_State *L, struct boxs *bs,int outfd, int req_id) {
	enum pkt_flags flags;
	size_t len;
	int32_t req;
	int rc;
...
static int muxrpc_write_sink(lua_State *L, struct boxs *bs, int infd, enum pkt_type ptype, int req_id) {
	int rc;
	fd_set rd;
	int sfd = bs->s;
	int maxfd = infd > sfd ? infd : sfd;
...
static int muxrpc_write_blob_add(lua_State *L, struct boxs *bs, int infd,int outfd, int req_id) {
	int rc;
	fd_set rd;
	int sfd = bs->s;
	int maxfd = infd > sfd ? infd : sfd;
...
static int muxrpc_duplex(lua_State *L, struct boxs *bs, int infd,int outfd, enum pkt_type in_ptype, int req_id) {
	int rc;
	fd_set rd;
	int sfd = bs->s;
	int maxfd = infd > sfd ? infd : sfd;
...
static int luamux(lua_State *L) {
	...
	methodstr = luaL_checkstring(L, 1);
	argument = luaL_checkstring(L, 2);
	...
	if (!typestr) {
		...
		if (len < 0) { 
			errl(1, "failed to read manifest file");
		}

		ssize_t type_len = json_get_value(manifest_buf, methodstr, &typestr);
		if (!typestr && errno == ENOMSG) {
			return luaL_error(L, "unable to find method '%s' in manifest", methodstr);
		}
		if (!typestr) {
			errl(1, "unable to read manifest.");
		}
	...
	if (strcmp(typestr, "sync") == 0) type = muxrpc_type_async;
		...
		errl(1, "type must be one of <async|sink|source|duplex>");
	}

	rc = method_to_json(method, sizeof(method), methodstr);
	if (rc < 0) {
		errl(0, "unable to convert method name");
	}
...
int main(int argc, char *argv[]) {
	...
	if (passthrough) {
		if (luafile) errx(1, "-a option conflicts with method");
	...
	bool implied_auth = key_arg || keypair_seed_str || shs_cap_key_str_arg;
...
do_tcp_connect:
	...
	// lua do file
	if (luaL_dofile(L, "prelude.lua") != LUA_OK) {
		const char *errmsg = lua_tostring(L, -1);
		err(1, "Lua prelude error: %s", errmsg);
	}
	if (luaL_dofile(L, luafile) != LUA_OK) {
		const char *errmsg = lua_tostring(L, -1);
		err(1, "Lua error: %s", errmsg);
	}
